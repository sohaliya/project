...
function setup() {
  ...
  movingRiver = createSprite(windowWidth-500,windowHeight-275,windowWidth+windowWidth*10000,windowHeight-175);
  movingRiver.addImage(riverimg);
  movingRiver.velocityX=-5;
  movingRiver.scale=1.55;

  invisibleGround=createSprite(windowWidth/2,windowHeight-590,windowWidth,5);
  ...
  monkey = createSprite(windowWidth-750,windowHeight-600,20,20);
  monkey.addImage(monkeyimg);
  monkeywin = createSprite(windowWidth-700,windowHeight-80,20,20);
  monkeywin.addImage(monkeywinimg);
  monkeywin.scale=0.45;
  monkeywin.visible=false;
  banana = createSprite(windowWidth-750,windowHeight-80,20,20);
...
function draw() {
  ...
  fill("white");
  textSize(30);
  text("score:" +score,windowWidth-125,20);
  ...
  if (time > 0 && gameState==="wait") { 
    textSize(100);
     ...
     text(" Navigate using LEFT and RIGHT",windowWidth/20,windowHeight/14);
     text("arrow keys and SPACE to move down",windowWidth/20,windowHeight/9);
    } 
  else if(gameState==="wait" && time===0) {
    textSize(100);
    textFont("bradley hand ITC");  
     text("press space to START", windowWidth/2-400,windowHeight/2); 
     check = 1;
    }
...
if(gameState==="end" && dummy===1){
  ...
  textFont("Broadway");  
  textSize(100);
  text("Got you",windowWidth/2-100,windowHeight/2);
  textSize(75);
  text("score "+score,windowWidth/2-100,windowHeight/2+100);
...
if(gameState==="end" && dummy===2){
  ...
  textFont("Broadway");  
  textSize(100);
  text("Too bad",windowWidth/2-100,windowHeight/2);
  monkey.velocityY=0;
  monkey.velocityX=0;
}
...
function spawnCroc(){
  if(frameCount % 200===0){
    var croc = createSprite(windowWidth,random(windowHeight-475,windowHeight-145),10,10);
    croc.addImage(crocodile);
    croc.velocityX = -10;
    croc.scale=0.5;
    croc.lifetime=windowWidth/10;
    crocGroup.add(croc);
...
function spawnTwig(){
  if(frameCount % 150===0){
    var twig = createSprite(windowWidth,random(windowHeight-475,windowHeight-145),10,10);
    twig.addImage(twigt);
    twig.velocityX = -10;
    twig.scale=0.5;
    twig.lifetime=windowWidth/10;
    twigGroup.add(twig);
...
function spawnLog1(){
  if(frameCount % 50===0){
    var l1 = createSprite(windowWidth,windowHeight-430,10,10);
    l1.addImage(log1);
    l1.velocityX = -10;
    l1.scale=0.5;
    l1.lifetime=windowWidth/10;
    logGroup1.add(l1);
...
function spawnLog2(){
  if(frameCount % 150===0){
    var l2 = createSprite(windowWidth,windowHeight-430,10,10);
    l2.addImage(log2);
    l2.velocityX = -20;
    l2.scale=0.5;
    l2.lifetime=windowWidth/10;
    logGroup1.add(l2);
...
function spawnLog3(){
  if(frameCount % 125===0){
    var l3 = createSprite(windowWidth,windowHeight-430,10,10);
    l3.addImage(log3);
    l3.velocityX = -15;
    l3.scale=0.5;
    l3.lifetime=windowWidth/10;
    logGroup1.add(l3);
...
function spawnLog4(){
  if(frameCount % 100===0){
    var l4 = createSprite(windowWidth,windowHeight-330,10,10);
    l4.addImage(log1);
    l4.velocityX = -15;
    l4.scale=0.5;
    l4.lifetime=windowWidth/10;
    logGroup1.add(l4);
...
function spawnLog5(){
  if(frameCount % 50===0){
    var l5 = createSprite(windowWidth,windowHeight-330,10,10);
    l5.addImage(log2);
    l5.velocityX = -20;
    l5.scale=0.5;
    l5.lifetime=windowWidth/10;
    logGroup1.add(l5);
...
function spawnLog6(){
  if(frameCount % 125===0){
    var l6 = createSprite(windowWidth,windowHeight-330,10,10);
    l6.addImage(log3);
    l6.velocityX = -10;
    l6.scale=0.5;
    l6.lifetime=windowWidth/10;
    logGroup1.add(l6);
...
function spawnLog7(){
  if(frameCount % 100===0){
    var l7 = createSprite(windowWidth,windowHeight-230,10,10);
    l7.addImage(log1);
    l7.velocityX = -20;
    l7.scale=0.5;
    l7.lifetime=windowWidth/10;
    logGroup1.add(l7);
...
function spawnLog8(){
  if(frameCount % 150===0){
    var l8 = createSprite(windowWidth,windowHeight-230,10,10);
    l8.addImage(log2);
    l8.velocityX = -10;
    l8.scale=0.5;
    l8.lifetime=windowWidth/10;
    logGroup1.add(l8);
...
function spawnLog9(){
  if(frameCount % 50===0){
    var l9 = createSprite(windowWidth,windowHeight-230,10,10);
    l9.addImage(log3);
    l9.velocityX = -15;
    l9.scale=0.5;
    l9.lifetime=windowWidth/10;
    logGroup1.add(l9);
